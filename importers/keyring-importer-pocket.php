...
function Keyring_Pocket_Importer() {
	...
	class Keyring_Pocket_Importer extends Keyring_Importer_Base {
		...
		function parse_pocket_title( $link ) {
			...
			// Poket may be able to parse the title ( resolved_title )
		...
		function insert_posts() {
			...
			if ( count( $this->posts ) === $skipped ) {
				...
				$this->set_option(
					'next_request_url',
					add_query_arg(
						array( 'offset' => $this->get_option('current_offset' ), 'since' => $this->get_latest_pocket_ts() ), $this->get_base_url() )
				);
			}
...
add_action( 'init', function() {
	...
	keyring_register_importer(
		...
		__( 'Import your links from Pocket ( getpocket.com )', 'keyring' )
	);
} );
